    ...
    showStartScreen() {
        ...
        const startScreenClick = (e) => {
            ...
            if (x >= this.canvas.width/2 - buttonWidth/2 && 
                ...
                // Play the start sound with maximum reliability - force multiple sources
                try {
                    console.log("Playing start sound - direct user interaction");
                    
                    // Try multiple sources in sequence for maximum compatibility
                    const tryPlaySound = async () => {
                        ...
                        for (const src of soundSources) {
                            try {
                                console.log(`Trying sound source: ${src}`);
                                const sound = new Audio(src);
                                sound.volume = 1.0;
                                
                                // Use load() before play() for better browser compatibility
                                await sound.load();
                                await sound.play();
                                
                                console.log(`Successfully played sound from: ${src}`);
                                return true;
                            } catch (err) {
                                console.log(`Failed to play from source ${src}:`, err);
                            }
                        }
                    ...
                    tryPlaySound().then(success => {
                        if (!success) {
                            ...
                            setTimeout(() => {
                                console.log("Retrying sound play after delay...");
                                tryPlaySound();
                            }, 200);
                        }
                    });
